...
class KinematicChain():
    ...
    def fkin(self, q):
        ...
        for step in self.steps:
            # T is transform w.r.t world frame
            T_ = T @ step.Tshift
            ...
            if step.type is JointType.REVOLUTE:
                ...
                R_revolute = Rotn(step.nlocal, q[step.dof])
                p_revolute = pzero()
                T_revolute = T_from_Rp(R_revolute, p_revolute)
                T = T_ @ T_revolute
                
            elif step.type is JointType.LINEAR:
                # Linear is a shift followed by a translation:
                R_linear = Reye()
                p_linear = q[step.dof] * step.nlocal
                T_linear = T_from_Rp(R_linear, p_linear)
                T = T_ @ T_linear
                ...
                R_fixed = Reye()
                p_fixed = pzero()
                T_fixed = T_from_Rp(R_fixed, p_fixed)
                T = T_ @ T_fixed
